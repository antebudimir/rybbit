...
**⚠️ Important:** You **must build locally** - pre-built Docker images won't work because Next.js bakes environment variables into the JavaScript bundle at build time. Each deployment needs its own build with custom configuration.
...
## Podman Alternative (Production)

Podman is a drop-in Docker replacement that's daemonless and runs rootless by default. It's fully compatible with the existing Docker Compose setup.

### Prerequisites

Install Podman and Podman Compose.

Start Podman socket (required for Docker Compose compatibility)

```bash
systemctl --user start podman.socket
systemctl --user enable podman.socket  # Optional: auto-start on boot
```
...
Podman can accumulate similar disk usage as Docker. Clean up regularly:
...
# Remove only images (keeps containers)
